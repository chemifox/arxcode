...
def create_ticket(caller, message, priority=3, queue_slug=settings.REQUEST_QUEUE_SLUG, kb_category=None,
    ...
    ticket = Ticket(title=optional_title, queue=queue, db_date_created=time_now(), submitter_email=email,
...
def add_followup(caller, ticket, message, mail_player=True):
    ...
    """
    try:
        new_followup = FollowUp(user_id=caller.id, date=time_now(aware=True), ticket=ticket, comment=message,
                                public=False)
        new_followup.save()
